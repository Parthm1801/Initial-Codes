...
I=(median(median(S))*eye(n))*9; %Evaluating I using median of the off-diagonal elements of Similarity matrix
S=S+I;
...
S=S+(eps*randn(N,N))*(max(S(:))-min(S(:)))*50; % Remove degeneracies
lam=0.51; % Set damping factor
...
for i=1:100 %number of iterations set to 100, can be changed depending on dataset
    ...
    A=(1-lam)*A+lam*Aold; % Dampen availabilitiesou
...
% scatter3(X,Y,Z,100,color,'filled');
